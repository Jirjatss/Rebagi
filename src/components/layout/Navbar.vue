<template>
  <div v-if="isNavbar" class="navbar bg-[#00AF91] fixed z-50 py-5 lg:px-28 px-10 font-mulish">
    ...
    <div class="navbar-end gap-3 font-bold">
      <a class="py-2 px-8 bg-[#00AF91] border-white border-2 rounded-lg text-white cursor-pointer hidden md:flex">Masuk</a>
      <a class="py-2 px-8 bg-white rounded-lg text-[#00AF91] cursor-pointer">Daftar</a>
    </div>
  </div>
  <div v-else-if="!isNavbar" class="navbar bg-transparent fixed z-50 py-5 lg:px-28 px-10 font-mulish" v-on:scroll="changeBg">
    <div class="navbar-start">
      <div class="dropdown">
        <button class="navbar-toggle md:hidden mx-3" :class="{ expanded: isExpanded }" @click="toggleExpand">
          <div class="bg-[#00AF91]">
